...
class SimpleKNN {
    ...
    train(features, labels) {
        if (!features || !features.length || !labels || !labels.length) return;
...
class SimpleLogisticRegression {
    ...
    trainOne(features, label) {
        if (!features || !features.length || !label) return;
...
class BaccaratMachineLearning {
    ...
    extractFeatures(rounds, windowSize) {
        if (!rounds || !rounds.length || rounds.length < windowSize) return null;
    ...
    predict(rounds, windowSize) {
        if (!this.isEnabled || !rounds || !rounds.length || rounds.length < windowSize) {
            return null;
        }
            ...
            if (data.mlData) {
                ...
                setTimeout(() => {
                    if (this.state.ml.featureHistory.length > 0) {
                        this.state.ml.featureHistory.forEach((item) => {
                            if (item.features && item.label) {
...
function init() {
    ...
    // إعداد عناصر الواجهة
    const windowSizeEl = el('windowSize');
    if (windowSizeEl) windowSizeEl.value = state.windowSize;

    const applyWindowBtn = el('applyWindow');
    if (applyWindowBtn) {
    ...
    const analysisModeEl = el('analysisMode');
    if (analysisModeEl) {
        analysisModeEl.value = state.analysisMode;
        analysisModeEl.onchange = (e) => {
    ...
    const confidenceSlider = el('confidenceSensitivity');
    if (confidenceSlider) {
        ...
        confidenceSlider.oninput = (e) => {
            ...
            const sensitivityEl = el('sensitivityValue');
            if (sensitivityEl) sensitivityEl.textContent = state.confidenceThreshold + '%';
            updateAll();
        };
    }

    const timeWeightingEl = el('timeWeighting');
    if (timeWeightingEl) {
        timeWeightingEl.value = state.timeWeighting;
        timeWeightingEl.onchange = (e) => {
    ...
    // إعدادات تعلم الآلة
    const mlToggle = el('mlToggle');
    if (mlToggle) {
        mlToggle.checked = state.useML;
        mlToggle.onchange = (e) => {
    ...
    const mlModelSelect = el('mlModelSelect');
    if (mlModelSelect) {
        mlModelSelect.value = state.mlModel;
        mlModelSelect.onchange = (e) => {
    ...
    // أزرار الإدخال
    const btnP = el('btnP');
    if (btnP) btnP.onclick = () => pushRound('P');
    
    const btnB = el('btnB');
    if (btnB) btnB.onclick = () => pushRound('B');
    
    const btnT = el('btnT');
    if (btnT) btnT.onclick = () => pushRound('T');
    
    const btnUndo = el('btnUndo');
    if (btnUndo) btnUndo.onclick = undoRound;
    ...
    if (btnExport) {
        btnExport.onclick = () => dataManager.exportData();
    }
    ...
    if (btnClearData) {
        btnClearData.onclick = () => dataManager.clearAllData();
    }
...
function frequencyAnalysis(rounds) {
    if (!rounds || !rounds.length) return { P: 33.3, B: 33.3, T: 33.3 };
...
function patternAnalysis(rounds) {
    if (!rounds || !rounds.length || rounds.length < 3) return { P: 0, B: 0, T: 0 };
...
function applyTimeWeighting(rounds, basePrediction) {
    if (state.timeWeighting === 'none' || !rounds || !rounds.length || rounds.length < 2) {
...
function enhancedPredict() {
    ...
    switch (state.analysisMode) {
        ...
        case 'advanced':
            ...
            finalPrediction = {
                P: basePrediction.P * 0.7 + (patternBoost.P || 0) * 0.3,
                B: basePrediction.B * 0.7 + (patternBoost.B || 0) * 0.3,
                T: basePrediction.T * 0.7 + (patternBoost.T || 0) * 0.3
            };
            break;
        ...
        case 'pattern':
            ...
            finalPrediction = {
                P: basePrediction.P * 0.4 + (patternBoost.P || 0) * 0.6,
                B: basePrediction.B * 0.4 + (patternBoost.B || 0) * 0.6,
                T: basePrediction.T * 0.4 + (patternBoost.T || 0) * 0.6
            };
            break;
        ...
        case 'ml':
            ...
            const mlPrediction = state.ml.predict(state.rounds, state.windowSize);
            const safeMlPrediction = mlPrediction || { P: 33.3, B: 33.3, T: 33.3 };

            const mlWeight = state.ml.getMLWeight();
            mlContribution = mlWeight * 100;

            finalPrediction = {
                P: basePrediction.P * (1 - mlWeight) + safeMlPrediction.P * mlWeight,
                B: basePrediction.B * (1 - mlWeight) + safeMlPrediction.B * mlWeight,
                T: basePrediction.T * (1 - mlWeight) + safeMlPrediction.T * mlWeight
            };
            break;
...
function updatePredictionDisplay(pred) {
    const predPctP = el('predPctP');
    const predPctB = el('predPctB');
    const predPctT = el('predPctT');
    
    if (predPctP) predPctP.textContent = pred.P + '%';
    if (predPctB) predPctB.textContent = pred.B + '%';
    if (predPctT) predPctT.textContent = pred.T + '%';
    ...
    if (finalCard) {
        let finalText = '—';
        if (pred.final === 'P') finalText = 'لاعب';
        else if (pred.final === 'B') finalText = 'مصرفي';
        else if (pred.final === 'T') finalText = 'تعادل';
        finalCard.textContent = finalText;
    }

    const mlContributionEl = el('mlContribution');
    if (mlContributionEl) {
        mlContributionEl.textContent = pred.mlContribution > 0 ?
...
function updateMLStatus() {
    ...
    const mlTrainingCountEl = el('mlTrainingCount');
    const mlWeightEl = el('mlWeight');
    const mlAccuracyEl = el('mlAccuracy');
    
    if (mlTrainingCountEl) mlTrainingCountEl.textContent = trainingCount;
    if (mlWeightEl) mlWeightEl.textContent = mlWeight.toFixed(0) + '%';
    if (mlAccuracyEl) mlAccuracyEl.textContent = mlAccuracy.toFixed(1) + '%';
    ...
    const mlOverallAccuracy = el('mlOverallAccuracy');
    if (mlOverallAccuracy) mlOverallAccuracy.textContent = mlAccuracy.toFixed(1) + '%';
}
...
function updateMLStats() {
    ...
    const pMLAccuracy = el('pMLAccuracy');
    const bMLAccuracy = el('bMLAccuracy');
    const tMLAccuracy = el('tMLAccuracy');
    
    if (pMLAccuracy) pMLAccuracy.textContent = (mlAccuracy * 0.9).toFixed(1) + '%';
    if (bMLAccuracy) bMLAccuracy.textContent = (mlAccuracy * 0.95).toFixed(1) + '%';
    if (tMLAccuracy) tMLAccuracy.textContent = (mlAccuracy * 0.7).toFixed(1) + '%';
}
...
function renderStats() {
    ...
    if (pTotal) pTotal.textContent = state.count.P;
    if (bTotal) bTotal.textContent = state.count.B;
    if (tTotal) tTotal.textContent = state.count.T;

    const pWin = el('pWin');
    const bWin = el('bWin');
    const tWin = el('tWin');
    
    if (pWin) pWin.textContent = state.win.P;
    if (bWin) bWin.textContent = state.win.B;
    if (tWin) tWin.textContent = state.win.T;

    const pLoss = el('pLoss');
    const bLoss = el('bLoss');
    const tLoss = el('tLoss');
    
    if (pLoss) pLoss.textContent = state.loss.P;
    if (bLoss) bLoss.textContent = state.loss.B;
    if (tLoss) tLoss.textContent = state.loss.T;

    const pPct = el('pPct');
    const bPct = el('bPct');
    const tPct = el('tPct');
    
    if (pPct) pPct.textContent = calculatePercentage(state.win.P, state.loss.P);
    if (bPct) bPct.textContent = calculatePercentage(state.win.B, state.loss.B);
    if (tPct) tPct.textContent = calculatePercentage(state.win.T, state.loss.T);

    const pAccuracy = el('pAccuracy');
    const bAccuracy = el('bAccuracy');
    const tAccuracy = el('tAccuracy');
    
    if (pAccuracy) pAccuracy.textContent = calculateAccuracy('P');
    if (bAccuracy) bAccuracy.textContent = calculateAccuracy('B');
    if (tAccuracy) tAccuracy.textContent = calculateAccuracy('T');
}
...
function updateAccuracyStats() {
    const currentAccuracyEl = el('currentAccuracy');
    const bestAccuracyEl = el('bestAccuracy');
    const averageAccuracyEl = el('averageAccuracy');
    
    if (state.accuracyHistory.length === 0) {
        if (currentAccuracyEl) currentAccuracyEl.textContent = '0%';
        if (bestAccuracyEl) bestAccuracyEl.textContent = '0%';
        if (averageAccuracyEl) averageAccuracyEl.textContent = '0%';
        return;
    }
    ...
    if (currentAccuracyEl) currentAccuracyEl.textContent = currentAccuracy.toFixed(1) + '%';
    if (bestAccuracyEl) bestAccuracyEl.textContent = bestAccuracy.toFixed(1) + '%';
    if (averageAccuracyEl) averageAccuracyEl.textContent = averageAccuracy.toFixed(1) + '%';
}
